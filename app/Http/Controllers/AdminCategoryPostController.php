...
class AdminCategoryPostController extends Controller
{
    ...
    {
        $category_posts = $this->category_post->paginate(5);
    ...
    public function post(Request $request)
    {
        ...
        if (empty($id)) {
            $this->category_post::FirstOrCreate($data);
        } else {
            $this->category_post::find($id)->update($data);
        }
        return redirect()->route('admin.category_posts.index');
    }

    public function delete($id)
    {
        try {
            $countPostByCategoryPost = $this->post->where('category_post_id', $id)->count();
            if ($countPostByCategoryPost == 0) {
